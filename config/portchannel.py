...
def add_portchannel(db, portchannel_name, min_links, fallback):
    ...
    if fallback != 'false':
        fvs['fallback'] = 'true'
    db.cfgdb.set_entry('PORTCHANNEL', portchannel_name, fvs)
...
def remove_portchannel(db, portchannel_name):
    """Remove port channel"""
    db.cfgdb.set_entry('PORTCHANNEL', portchannel_name, None)
...
def add_portchannel_member(db, portchannel_name, port_name):
    ...
    if is_interface_mirror_dst_port(db.cfgdb, port_name):
        ctx.fail("{} is configured as mirror destination port".format(port_name))
    db.cfgdb.set_entry('PORTCHANNEL_MEMBER', (portchannel_name, port_name),
            {'NULL': 'NULL'})
...
def del_portchannel_member(db, portchannel_name, port_name):
    ...
    db.cfgdb.set_entry('PORTCHANNEL_MEMBER', (portchannel_name, port_name), None)
    db.cfgdb.set_entry('PORTCHANNEL_MEMBER', portchannel_name + '|' + port_name, None)
